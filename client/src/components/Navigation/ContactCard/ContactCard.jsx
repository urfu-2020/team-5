...
export const ContactCard = ({ id, name, isOnline, message, countUnreadMessage, avatarUrl }) => (
  <div className="card">
    <div className="card__avatar">
      <svg role="none" style={{ height: '64px', width: '64px' }}>
        <defs>
          <mask id="circle_online">
            <circle cx="32" cy="32" fill="white" r="32"></circle>
            <circle cx="56" cy="56" fill="black" r="7"></circle>
          </mask>
      ...
      {
        isOnline ? (<div className="badge"></div>) : null
      }
    </div>
    <div className="card__content">
      <h3 className="dialogHeader">{name}</h3>
      {
        message.sender !== id ? (
          <>
